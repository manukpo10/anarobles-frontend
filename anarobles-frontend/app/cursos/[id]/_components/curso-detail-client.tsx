...
import { getCursoById, updateLessonProgress, getLessonProgress, getCursos, fetchCursoByIdFromAPI, estoyInscripto, inscribirEnCursoAPI, completarLeccion, type Curso, type Lesson, type QuizQuestion } from "@/lib/data"
...
export function CursoDetailClient({ id }: { id: string }) {
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Inicio", "item": "https://anaceciliarobles.com" },
      { "@type": "ListItem", "position": 2, "name": "Cursos", "item": "https://anaceciliarobles.com/cursos" },
      { "@type": "ListItem", "position": 3, "name": curso.title, "item": `https://anaceciliarobles.com/cursos/${curso.id}` }
    ]
  }
              ...
              <div className="p-6 md:p-8">
                ...
                {currentLesson.type === "pdf" && currentLesson.pdfUrl && (
                  <div className="mb-6 h-[60vh] overflow-hidden rounded-xl border border-gray-200">
                    <iframe src={currentLesson.pdfUrl} className="h-full w-full" title={currentLesson.title} />
                  </div>
                )}
                  ...
                  <div className="rounded-xl bg-gray-50 p-4">
                    ...
                    <div className="space-y-2">
                      {currentLesson.resources.map((res, idx) => (
                        <a key={idx} href={res.url} download className="flex items-center gap-3 rounded-lg p-3 transition-colors hover:bg-gray-100">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                  ...
                  {currentVideoUrl ? (
                    <div className="relative h-full w-full">
                      {isYouTubeUrl(currentVideoUrl) ? (
                        ...
                        <video src={currentVideoUrl} controls autoPlay className="h-full w-full object-cover">
                          Tu navegador no soporta el elemento de video.
                        </video>
                      )}
                      <button
                    ...
                    <div className="relative h-full w-full">
                      {isYouTubeUrl(curso.introVideoUrl) ? (
                        ...
                        <video src={curso.introVideoUrl} controls autoPlay className="h-full w-full object-cover">
                          Tu navegador no soporta el elemento de video.
                        </video>
                      )}
                      <button
                    ...
                    <>
                      <Image src={curso.image} alt={curso.title} fill className="object-cover" />
                ...
                <div className="mt-6 flex items-center gap-4">
                  <div className="relative h-12 w-12 overflow-hidden rounded-full bg-muted">
                    <Image src="/artist-portrait.jpg" alt="Ana Cecilia Robles" fill className="object-cover" />
                  </div>
                  <div>
                ...
                <p className="mt-8 leading-relaxed text-muted-foreground">{curso.description}</p>
              </motion.div>
              ...
              >
                <h2 className="mb-6 font-serif text-2xl font-medium text-foreground">Contenido del curso</h2>
                      ...
                      <div key={module.id} className="rounded-xl border border-border/30 bg-background overflow-hidden">
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
                              <span className="font-serif text-lg font-medium text-primary">{moduleIndex + 1}</span>
                            </div>
                            <div>
                            ...
                            {module.lessons.map((lesson) => (
                              ...
                              >
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    if (isEnrolled) toggleLesson(lesson.id)
                                  }}
                                  className="shrink-0"
                                  disabled={!isEnrolled}
                                >
                                ...
                                <div className="flex-1">
                                  <p className={`font-medium ${completedLessons.has(lesson.id) ? "text-muted-foreground line-through" : "text-foreground"}`}>
                                    {lesson.title}
                                  </p>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-32 space-y-6">
                ...
                >
                  ...
                  >
                    {isEnrolled ? (
                      <><Check className="h-5 w-5" /> Continuar aprendiendo</>
                    ) : (
                      <>Inscribirme ahora <ArrowRight className="h-5 w-5" /></>
                    )}
                  </motion.button>
              ...
              >
                <Link href={`/cursos/${related.id}`} className="group block">
                  ...
                  <div className="mt-4">
                    ...
                    <p className="mt-2 font-semibold text-primary">${formatPrice(related.precio)}</p>
                  </div>
                </Link>
              </motion.div>
            ))}
      ...
      <section className="border-t border-border/30 py-12">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex items-center justify-between">
            {prevCurso ? (
              <Link href={`/cursos/${prevCurso.id}`} className="group flex items-center gap-4 text-left transition-colors hover:text-primary">
            ...
            {nextCurso && (
              <Link href={`/cursos/${nextCurso.id}`} className="group flex items-center gap-4 text-right transition-colors hover:text-primary">
                <div>
