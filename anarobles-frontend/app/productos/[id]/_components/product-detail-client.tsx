...
import { fetchProductByIdFromAPI, fetchProductsFromAPI, Product } from "@/lib/data"
...
export function ProductDetailClient({ id }: { id: string }) {
  ...
  const breadcrumbJsonLd = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": "Productos", "item": "https://anaceciliarobles.com/productos" },
      { "@type": "ListItem", "position": 3, "name": product.name, "item": `https://anaceciliarobles.com/productos/${product.id}` }
    ]
  }
                ...
                >
                  {details.technique && (
                    <div className="flex items-center gap-3 rounded-xl bg-muted/50 p-4">
                      ...
                      <div>
                        <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Técnica</p>
                        <p className="font-medium text-foreground">{details.technique}</p>
                      </div>
                    </div>
                  )}
                  {details.dimensions && (
                    <div className="flex items-center gap-3 rounded-xl bg-muted/50 p-4">
                      ...
                      <div>
                        <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Dimensiones</p>
                        <p className="font-medium text-foreground">{details.dimensions}</p>
                      </div>
                    </div>
                  )}
                  {details.material && (
                    <div className="flex items-center gap-3 rounded-xl bg-muted/50 p-4">
                      ...
                      <div>
                        <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Material</p>
                        <p className="font-medium text-foreground">{details.material}</p>
                      </div>
                    </div>
                  )}
                </motion.div>
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {relatedProducts.map((relatedProduct, index) => (
                ...
                >
                  <Link href={`/productos/${relatedProduct.id}`} className="group block">
                    <motion.div
