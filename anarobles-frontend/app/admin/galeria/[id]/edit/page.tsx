...
export default function EditarObraPage() {
  ...
  useEffect(() => {
    const loadObra = async () => {
      ...
      const obra = await fetchAdminObraByIdAPI(token, obraId)
      if (obra) {
        setFormData({
          titulo: obra.titulo,
          ...
          precio: obra.precio !== undefined ? String(obra.precio) : "",
          descripcion: obra.descripcion || "",
        })
        setImagen({ url: obra.imagen, width: obra.imgW, height: obra.imgH })
      } else {
        toast.error("Error", { description: "No se pudo cargar la obra" })
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      toast.error("Error", { description: "Necesitás estar logueada para editar obras" })
      return
    }
    if (!imagen) {
      toast.error("Error", { description: "La obra necesita una imagen" })
      return
    }

    setIsLoading(true)
    try {
      const updated = await actualizarObraAPI(token, params.id as string, {
      ...
      if (updated) {
        toast.success("Obra editada correctamente ✓", { description: "Guardada en el servidor" })
        setTimeout(() => {
      ...
      toast.error("Error", { description: "No se pudo actualizar la obra" })
    } catch (error) {
      toast.error("Error", { description: "No se pudo actualizar la obra" })
    } finally {
      setIsLoading(false)
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <motion.div variants={itemVariants} className="space-y-6">
              ...
              >
                {isLoading ? "Actualizando..." : "Actualizar Obra"}
              </Button>
            </motion.div>
          </div>
        </form>
