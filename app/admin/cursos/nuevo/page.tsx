...
export default function NuevoCursoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (token) {
        const nuevoCurso = await crearCursoAPI(token, {
          title: formData.title,
          description: formData.description,
          precio: parseFloat(formData.precio),
          image: formData.image || "/artwork-1.jpg",
          category: formData.category || "Pintura",
      ...
      const newCurso = {
        ...
        ...formData,
        precio: parseFloat(formData.precio),
        introVideoUrl: (formData as any).introVideoUrl || "",
      ...
      <motion.div variants={itemVariants} className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Completá la información del curso y sus lecciones
          </p>
        </div>
      </motion.div>
