...
export default function AdminProductosPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (token) {
      const ok = await eliminarProductoAPI(token, id)
      if (ok) {
        const data = await fetchAdminProductos(token)
        setProductsList(data)
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {product.featured && (
                            <span className="mr-2 inline-flex items-center gap-1 rounded-full bg-secondary/20 px-2 py-0.5 text-xs font-medium text-secondary">
                              Destacado
                            </span>
                          )}
                        </p>
